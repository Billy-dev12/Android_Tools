...
1. Pasang Mono di Linux Anda:
   - **Arch Linux:** `sudo pacman -S mono`
   - **Ubuntu/Debian:** `sudo apt install mono-devel`
2. Masuk ke direktori `gui_csharp` dan jalankan perintah kompilasi:
   ```bash
   mcs -target:winexe -out:AndroidToolsGUI.exe -reference:System.Windows.Forms -reference:System.Drawing Program.cs Form1.cs Form1.Designer.cs
   ```
   Perintah ini akan menghasilkan berkas **`AndroidToolsGUI.exe`** yang siap dijalankan di Windows.
